...
local function POI_SetOptions(frame, type, poi)
  ...
  if type == "mapLink" then
    ...
    frame:SetScript(
      ...
      function()
        ...
        if db.devMode then
          GameTooltip:AddLine(frame.poi.connectionIndex, 1, 1, 1, 1)
        end
        GameTooltip:Show()
        frame.HighlightTexture:Show()
      end
          ...
          for _, blip in pairs(blips) do
            if blip.data.id == poi.npcId then
              ...
              if isBlipSameWeek then
                ...
                frame.animatedLine =
                  MDT:ShowAnimatedLine(
                  MDT.main_frame.mapPanelFrame,
                  frame,
                  blipFrame,
                  nil,
                  nil,
  ...
  if type == "tuskarrCage" then
    frame:SetSize(25, 25)
    frame.Texture:SetSize(25, 25)
    frame.HighlightTexture:SetSize(25, 25)
    frame.HighlightTexture:SetAtlas("MajorFactions_MapIcons_Tuskarr64")
    frame.Texture:SetAtlas("MajorFactions_MapIcons_Tuskarr64")
    frame:SetScript(
      "OnClick",
      function()
      end
    ...
    frame:SetScript(
      ...
      function()
        GameTooltip:SetOwner(UIParent, "ANCHOR_CURSOR")
        GameTooltip:AddLine(string.format(L["tuskarrCage"], "\n"), 1, 1, 1, 1)
        GameTooltip:Show()
      end
    )
    frame:SetScript(
  ...
  if type == "mechagonBot" then
    ...
    frame.Texture:SetVertexColor(
      botOptions[poi.botType].color.r,
      botOptions[poi.botType].color.g,
      botOptions[poi.botType].color.b,
      botOptions[poi.botType].color.a
    )
    -- if db.devMode and poi.botTypeIndex then
    --     frame.Texture:SetVertexColor(0.45,0.45,0.45,0.6)
    -- end
    frame.HighlightTexture:SetVertexColor(
      botOptions[poi.botType].color.r,
      botOptions[poi.botType].color.g,
      botOptions[poi.botType].color.b,
      botOptions[poi.botType].color.a
  ...
  if frame.Texture then
    frame.Texture:SetSize(frame.Texture:GetWidth() * scale, frame.Texture:GetHeight() * scale)
  end
  if frame.HighlightTexture then
    frame.HighlightTexture:SetSize(
      frame.HighlightTexture:GetWidth() * scale,
      frame.HighlightTexture:GetHeight() * scale
    )
  end

  if db.devMode then
    POI_SetDevOptions(frame, poi)
  end
end
...
function MDT:POI_UpdateAll()
  ...
  for poiIdx, poi in pairs(pois) do
    if
      (not (poi.type == "nyalothaSpire" and (db.currentSeason ~= 4 or db.currentDifficulty < 10))) and
        ((not poi.weeks) or poi.weeks[week]) and
        (not poi.season or poi.season == db.currentSeason) and
        (not poi.difficulty or poi.difficulty <= db.currentDifficulty)
     then
      local poiFrame = framePools:Acquire(poi.template)
      if poiFrame.playerAssignmentString then
        poiFrame.playerAssignmentString:Hide()
      end
      ...
      if not poiFrame.defaultHidden or db.devMode then
        poiFrame:Show()
      end
...
local function animateLine(self, elapsed)
  local totalDistance =
    math.sqrt(math.pow(self.frameTwoX - self.frameOneX, 2) + math.pow(self.frameTwoY - self.frameOneY, 2))
...
function MDT:ShowAnimatedLine(parent, frame1, frame2, sizeX, sizeY, gap, color, speed, selected, animatedLine)
  ...
  if selected then
    animatedLine.color = {0.5, 1, 0.1, 1}
  end
  ...
  local totalDistance =
    math.sqrt(
    math.pow(animatedLine.frameTwoX - animatedLine.frameOneX, 2) +
...
function MDT:DrawAllAnimatedLines()
  ...
  for _, blip in pairs(MDT:GetDungeonEnemyBlips()) do
    ...
    elseif blip.data.corrupted and blip.selected then
      ...
      blip.animatedLine =
        MDT:ShowAnimatedLine(MDT.main_frame.mapPanelFrame, connectedFrame, blip, nil, nil, nil, nil, nil, blip.selected)
...
function MDT:FindConnectedDoor(npcId, numConnection)
  ...
  local connection =
    riftOffsets and riftOffsets[npcId] and riftOffsets[npcId].connections and
    riftOffsets[npcId].connections[numConnection or #riftOffsets[npcId].connections] or
    nil
  if connection then
