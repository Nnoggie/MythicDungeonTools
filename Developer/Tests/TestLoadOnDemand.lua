...
local function testFunc()
  ...
  assert(type(_G.MythicDungeonToolsAPI.IterateEnemies) == "function", "Enemy metadata API is missing")
  assert(type(_G.MythicDungeonToolsAPI.OpenEnemyInfo) == "function", "Enemy navigation API is missing")
  local iterator = assert(_G.MythicDungeonToolsAPI:IterateEnemies())
  local enemy = assert(iterator(), "Enemy metadata is empty")
  assert(enemy.npcID and enemy.name and type(enemy.spellIDs) == "table", "Enemy metadata is incomplete")
  assert(enemy.clones == nil and enemy.spells == nil, "Enemy metadata exposes internal tables")
