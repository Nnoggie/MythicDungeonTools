...
def get_dungeon_id(boss_info, mobHits):
    ...
    if not len(boss_info) > 0:
        return print(
            "Error: Combat log does not contain any boss fights. A boss fight required for collecting count."
        )
    ...
    CriteriaIDs = db["criteria"][
        (db["criteria"].Asset.isin(DungeonEncounterIDs)) & (db["criteria"].Type == 165)
    ].ID.values
    ...
    if len(challenge_at_end) > 0:
        ...
        all_npcids = mobHits[
            mobHits.destName.isin(boss_info.sourceName.to_list())
        ].npcID.to_list()
    ...
    for boss_name in boss_info.sourceName.to_list():
        parent_dungeons = db["criteriatree"][
            db["criteriatree"].Description_lang.str.startswith(boss_name, na=False)
        ].Parent
        if len(parent_dungeons) == 0:
            ...
            mythic_regular = db["criteriatree"][
                (
                    (
                        db["criteriatree"].Description_lang.str.contains(
                            "Dungeon.*Challenge", na=False
                        )
                    )
...
def get_count_table(ID, db_files=None):
    ...
    if db_files is None:
        db_files = db
    dungeon_forces = db_files["criteriatree"][
        (
            (db_files["criteriatree"].Parent == ID)
            & (db_files["criteriatree"].Description_lang == "Enemy Forces")
        )
    ]
    enemy_forces = db_files["criteriatree"][
        db_files["criteriatree"].Parent == int(dungeon_forces.ID)
    ].copy()
    enemy_forces["npcID"] = [
        int(db_files["criteria"][db_files["criteria"].ID == ID].Asset)
        for ID in enemy_forces.CriteriaID
    ]
...
def make_aura_check_GUID_list(CL, aura):
    ...
    """
    GUID_list = CL.loc[
        ((CL.event == "SPELL_AURA_APPLIED") & (CL.spellName == aura))
    ].destGUID.values
    return GUID_list
...
def create_mapPOIs(combatlog):
    ...
    """
    pois = combatlog.loc[
        (combatlog.event == "MAP_CHANGE") | (combatlog.event == "WORLD_MARKER_PLACED"),
        [
            "event",
            "sourceGUID",
            "sourceName",
    ...
    pois.loc[pois.event == "MAP_CHANGE", "UiMapID"] = pois[
        pois.event == "MAP_CHANGE"
    ].MapID
...
def create_map_doors(pois):
    ...
    blue_pois = pois[
        (pois.event == "MAP_CHANGE")
        | ((pois.event == "WORLD_MARKER_PLACED") & (pois.markerid == "0"))
    ].copy()  # Blue world marker
    blue_pois.loc[pois.event == "MAP_CHANGE", "to_UiMapID"] = pois[
        pois.event == "MAP_CHANGE"
    ].MapID
    ...
    green_pois = pois[
        (pois.event == "MAP_CHANGE")
        | ((pois.event == "WORLD_MARKER_PLACED") & (pois.markerid == "1"))
    ].copy()  # Green world marker
    green_pois.loc[pois.event == "MAP_CHANGE", "to_UiMapID"] = pois[
        pois.event == "MAP_CHANGE"
    ].MapID
    ...
    for index in green_pois[green_pois.event == "WORLD_MARKER_PLACED"].index.to_list():
        map_changes = green_pois[
            (
                (green_pois.event == "MAP_CHANGE")
                & (green_pois.UiMapID != green_pois.loc[index, "UiMapID"])
            )
        ].loc[:index]
    ...
    door_pois.drop(
        door_pois.index[door_pois.event == "MAP_CHANGE"], axis=0, inplace=True
    )
    ...
    blue_sublevels = door_pois.loc[
        (door_pois.connectionIndex.isin(door_pairs)) & (door_pois.markerid == "0"),
        "from_sublevel",
    ].to_list()
    green_sublevels = door_pois.loc[
        (door_pois.connectionIndex.isin(door_pairs)) & (door_pois.markerid == "1"),
        "from_sublevel",
    ].to_list()
...
if __name__ == "__main__":
    ...
    timesplitdf = pd.DataFrame.from_records(
        timesplit, columns=["date", "time", "remove", "event"]
    )
    ...
    mobHits = CL.loc[
        (CL.event == "SPELL_DAMAGE")
        & (~CL.destGUID.str.startswith("Player", na=True))
        & (  # Filters out damage events against the player
            ~CL.ownerGUID.str.startswith("Player", na=True)
        )
        & (CL.destName != "Unknown"),  # Filters out damage events against player pets
        [
            "destGUID",
            "ownerGUID",
            "destName",
    ...
    mobHits = mobHits.astype(
        {"UiMapID": int, "maxHP": int, "level": int}
    )  # not working for Tazavesh Streets
    mobHits = mobHits.astype({"UiMapID": int})

    unique_UiMapIDs = mobHits.UiMapID.unique().tolist()

    db["uimapassignment"].rename(
        columns={
            "MapID": "continentID",
            "Region[0]": "ymin",
            "Region[1]": "xmax",
    ...
    db["uimapassignment"].xmin = (
        db["uimapassignment"].xmin * -1
    )  # Multiply by -1 because of blizzard coords
    db["uimapassignment"].xmax = (
        db["uimapassignment"].xmax * -1
    )  # Multiply by -1 because of blizzard coords
    db["map"].rename(
        columns={
            "ID": "continentID",
    ...
    map_extent = (
        db["map"]
        .merge(db["uimapassignment"], on="continentID")[info_columns]
        .copy()
        .round(1)
        .drop_duplicates()
    )
    ...
    mobHits["mobcount"] = [
        get_npc_count(npcID, regular_count) for npcID in mobHits.npcID
    ]
    ...
    deleted_mobs = mobHits.loc[
        (mobHits.ownerGUID.str.startswith("Creature"))
        & (mobHits.maxHP < 20000)
        & (mobHits.mobcount == 0)
    ]
    mobHits.drop(
    ...
    print(
        "{} enemy pets deleted due to low health (sub {}) and no count.".format(
            len(deleted_mobs), HP_threshold
        )
    )
    ...
    if toggle_door_mapping:
        ...
        for sublevel in door_pois["from_sublevel"].unique():
            ...
            for idx, (_, door) in enumerate(
                door_pois[door_pois.from_sublevel == sublevel].iterrows()
            ):
    ...
    for unique_npc_index, unique_npcID in enumerate(mobHits.npcID.unique()):
        ...
        for npc_index, npc in enumerate(
            mobHits[mobHits.npcID == unique_npcID].itertuples()
        ):
        ...
        if npc.destName in boss_info.sourceName.to_list():
            encounterID, instanceID = get_additional_boss_info(
                npc.destName, unique_UiMapIDs
            )
    ...
    print(
        "This dungeon requires {} count to complete and has {} total count. You need to clear {}% of the dungeon.".format(
            total_count,
            mobHits.mobcount.sum(),
            int(round(total_count / mobHits.mobcount.sum(), 2) * 100),
        )
    )
