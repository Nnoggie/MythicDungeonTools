...
def get_latest_version(filename, game_version=None):
    ...
    for version in ["Retail", "PTR", "Beta"]:
        latest_build = re.search(
            f"<td>{version}</td>\\s*<td>(.*?)</td>", page_text
        ).group(1)
...
def load_db_files(wowtools_files=None, game_version=None):
    ...
    for file in wowtools_files:
        try:
            ...
            # if file_versions[file].startswith(
            #     game_version
            # ):  # and file_age < max_file_age:
            ...
            print(f"File {file} not found, will not download anything until wago works")
            # db[file], version = get_latest_version(file, game_version)
            # file_versions[file] = version
...
if (
    __name__ == "__main__"
):  # Meaning if this file is run itself and not imported by another script
    print(
        "If you want to map with combat log, but scrape and import the displayIds and creatureType later,\n"
