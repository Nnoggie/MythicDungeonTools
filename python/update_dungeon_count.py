...
GAME_VERSION = "10.1.0"  # <--- Set this variable
...
def fix_split_count_db(dungeon_id, true_count_table):
    ...
    """
    split_dungeons = {67227: [67283]}  # Siege of Boralus
    if dungeon_id in split_dungeons.keys():
        for other_id in split_dungeons[dungeon_id]:
            other_table = get_count_table(other_id, db)
            true_count_table = pd.concat(
                (
                    true_count_table,
                    other_table[~other_table.index.isin(true_count_table.index)],
                )
            )
...
def update_count(match, dungeon_count_table):
    ...
    if len(dungeon_count_table[dungeon_count_table.index == info["id"]]) > 0:
        ...
        true_count = dungeon_count_table[
            dungeon_count_table.index == info["id"]
        ].values[0][0]
    else:
    ...
    if info["id"] in unexplainable_count.keys():
        ...
        if unexplainable_count[info["id"]]["hardcoded_count"] != info["count"]:
            ...
            return match.group().replace(
                f'["count"] = {info["count"]}', f'["count"] = {new_count}'
            )
        ...
        if true_count == 0:
            print(
                f"    {npc_name} with id {info['id']} has been updated: {info['count']} -> {true_count}"
            )
            ...
            return match.group().replace(
                f'["count"] = {info["count"]}', f'["count"] = {true_count}'
            )
...
def update_total_count(match, total_count):
    ...
    if total_count != int(match.group(1)):
        print(
            f"    Total dungeon count has been updated: {match.group(1)} -> {total_count}"
        )
        return match.group().replace(
            f"normal = {match.group(1)}", f"normal = {total_count}"
        )
...
def update_file(fullpath):
    ...
    with open(fullpath, "r") as file:
        ...
        dungeon_enemies_true_count = pattern_enemy_match.sub(
            lambda match: update_count(match, true_mob_count), dungeon_enemies
        )
        ...
        true_count_string = pattern_count_value.sub(
            lambda match: update_total_count(match, true_total_count), total_count
        )

        updated_text = pattern_dungeonTotalCount.sub(true_count_string, file_text)
        updated_text = pattern_dungeonEnemies.sub(
            dungeon_enemies_true_count, updated_text
        )
...
if __name__ == "__main__":
    ...
    pattern_dungeon_name = re.compile(
        r"MDT\.dungeonList\[dungeonIndex\] = L\[\"([\w\S ]+)\"]"
    )
    pattern_empty_dungeon_file = re.compile(
        r"MDT\.dungeonEnemies\[dungeonIndex\] = {\n?}"
    )
    pattern_dungeonEnemies = re.compile(
        r"MDT\.dungeonEnemies\[dungeonIndex\] = \{[\s\S]*};"
    )
    pattern_enemy_match = re.compile(
        r"{[\s\S]+?\"(id|count)\"\D*([\d]+).*[\s\S]+?\"(id|count)\"\D*([\d]+)[\s\S]+?}"
    )
    ...
    for expansion in EXPANSIONS:
        ...
        for dungeon_filename in dungeon_filenames:
            if (
                dungeon_filename.endswith(".lua")
                and dungeon_filename != "overrides.lua"
            ):
